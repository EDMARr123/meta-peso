...
function montarConteudo(rca) {
  ...
  const linhas = ordem_categorias.map(chave => {
    ...
    const classeDif = diferenca >= 0 ? "dif-pos" : "dif-bad";
    const textoDif = (diferenca >= 0 ? "+" : "") + fmtPeso(diferenca) + " kg";
    return `<tr>
  ...
  return `
    <div class="panel">
      <table class="breakdown">
        <thead><tr><th>Categoria</th><th>Peso</th><th>Meta Peso</th><th>Diferença</th><th>Positivação</th><th>Meta Positivação</th></tr></thead>
        <tbody>${linhas}</tbody>
      </table>
...
// Se o Edmar editar a Meta Posit no Painel Performance (melhoria_salarial)
// numa ABA JÁ ABERTA deste painel, o navegador dispara "storage" — assim
// a Meta Positivação atualiza sozinha aqui, sem precisar dar F5.
window.addEventListener("storage", (e) => {
  if (e.key === CHAVE_OVERRIDES_DEPARTAMENTOS) renderizarRca();
});
